...
- **2.0.2** — Fixed second Companion 5.x init crash: `TypeError: Cannot convert undefined or null to object at sanitisePresetDefinitions`. API v2 changed `setPresetDefinitions()` to a **two-argument** call — `(structure, presets)` — and moved the `category` field off individual presets into a top-level section structure. `presets.js` now returns `{ structure, presets }`; `main.js` spreads both. New regression suite `test/api-v2-preset-shape.test.js` locks this shape in. 18 test suites now green.
- **2.0.1** — Fixed Companion 5.x init crash: `variables.js` was exporting an array (API v1 shape) but API v2 requires an object keyed by variable id. Updated variables and all consuming tests; added `test/api-v2-variable-shape.test.js` to prevent regression.
